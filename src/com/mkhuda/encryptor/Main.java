...
public class Main extends JFrame {
	...
	public Main() {
		setResizable(false);
		setTitle("JVE - Java Video Encryptor v.1.1");
		...
		contentPane.setLayout(new FormLayout(
				new ColumnSpec[] { ColumnSpec.decode("130px"),
						ColumnSpec.decode("171dlu:grow"), }, new RowSpec[] {
						RowSpec.decode("40dlu"), RowSpec.decode("23px"),
		...
		contentPane.add(button, "2, 5, left, center");

		lblmkhuda = DefaultComponentFactory.getInstance().createTitle(
				"@mkhuda (2015)");
	...
	public static void encrypt() throws Exception {
		...
		if (!(bonjour.length() == 16)) {
			JOptionPane.showMessageDialog(null,
					"Code must 16 length character. (Text length total: "
							+ bonjour.length() + ")");
		} else {
			JOptionPane.showMessageDialog(null, "Converting: " + source
					+ " => " + out);
			final byte[] buf = new byte[8192];
			final Cipher c = Cipher.getInstance("AES/CFB/NoPadding");
			c.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(bonjour.getBytes(),
					"AES"), new IvParameterSpec(new byte[16]));
			final InputStream is = new FileInputStream(source);
			final OutputStream os = new CipherOutputStream(
